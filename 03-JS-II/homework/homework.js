...
function operadoresLogicos(num1, num2, num3) {
  ...
  if( num1>0 && num2>0 && num3>0 && num1>num2 && num1>num3 ){return "Número 1 es mayor y positivo"}
  else if(num1<0 || num2<0 || num3<0){return "Hay negativos"}
  else if(num1==0||num2==0||num3==0){return "Error"}
  else if(num3>num2 && num3>num1){num3++; return num3}
  else{return false}
}
...
function esPrimo(numero) {
  ...
  for(let i=numero-1;i>1;i--)
    {
        if(numero%i==0){bl=false}
    }
    if(numero==0){bl=false}
    else if(numero==1){bl=false}
    return bl
}
...
function esVerdadero(valor){
  ...
  if(valor==true){return valor}else{ return valor}
...
function tieneTresDigitos(numero){
  ...
  //Escribe tu código aquí
  N=numero.toString()
  if(N.lengh==3){return true}else{return false}
...
function doWhile(numero) {
  ...
  contador=0;
  while(contador<8){numero=numero+5; contador++}
  return numero
}
